...
def AES(eng, resource_check):
    ...
    for i in range(14):
        ...
        if(i!=0):
            if(i%2 == 0):
                Keyshedule(eng, k, i, y, t, z, i, k_i, resource_check) #i=2, k_i=1, _one
                CNOT32(eng, k[96:128], k[64:96])
                CNOT32(eng, k[64:96], k[32:64])
                CNOT32(eng, k[32:64], k[0:32])
            else:
                Keyshedule(eng, k, i, y_two, t_two, z_two, i, k_i, resource_check) #i=1, k_i=0, _two, i=3, k_i=2, _two
    ...
    if (k_i % 2 == 0):
        for j in range(4): #22 68 18
             k[224 + 8 * j: 224 + 8 * (j + 1)] = Sbox(eng, new_k0[8 * j:8 * (j + 1)], y[432+27*j:432+27*(j+1)],
                                         t[1008+63*j:1008+63*(j+1)], z[480+30*j:480+30*(j+1)], k[224 + 8 * j: 224 + 8 * (j + 1)], 1, round, resource_check)
    else:
        for j in range(4): #22 68 18
