...
def AES(eng, resource_check):
    ...
    t_in0 = eng.allocate_qureg(71)
    t_in1 = eng.allocate_qureg(71)
    t_in2 = eng.allocate_qureg(71)
    t_in3 = eng.allocate_qureg(71)
    ...
    for p in range(5):
        ...
        if(i != 0):
            ...
            Clean_ancilla(eng, t0, t1, t2, t3, temp_k, y_two, t_two, z_two, resource_check)
        ...
        if(i!= 9):
            Clean_ancilla(eng, t0, t1, t2, t3, temp_k, y, t, z, resource_check)
        Keyshedule(eng, k, i, y_two, t_two, z_two, i, resource_check)
        ...
        SBox_bp12_all(eng, x0, x1, x2, x3, y_two, t_two, z_two, s1, i, resource_check)
        x0, x1, x2, x3 = Shiftrow(eng, x0, x1, x2, x3)
...
def Clean_ancilla(eng, x0, x1, x2, x3, temp_k, y, t, z, resource_check):
...
def Keyshedule(eng, k, i, y, t, z, round, resource_check):
    ...
    for j in range(4): #22 68 18
         k[96 + 8 * j: 96 + 8 * (j + 1)] = Sbox(eng, new_k0[8 * j:8 * (j + 1)], y[432+27*j:432+27*(j+1)],
                                         t[1008+63*j:1008+63*(j+1)], z[480+30*j:480+30*(j+1)], k[96 + 8 * j: 96 + 8 * (j + 1)], 1, round, resource_check)
    for j in range(8):
        if ((Rcon[i] >> j) & 1):
            X | k[120+j]
...
#def Sbox_bp12(eng, x, y, t, z, s, flag, round, resource_check):
