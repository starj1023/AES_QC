...
def AES(eng, resource_check):
    ...
    for p in range(4):
        ...
        if (i % 2 == 0):
            ...
            Keyshedule(eng, k, i, y_two, t_two, z_two, i, k_i, ancilla, resource_check)
...
def Keyshedule(eng, k, i, y, t, z, round, k_i, ancilla, resource_check):
    ...
    for j in range(4): #22 68 18
         k[160 + 8 * j: 160 + 8 * (j + 1)] = AND_KS_Sbox_T4(eng, new_k0[8 * j:8 * (j + 1)], y[432+27*j:432+27*(j+1)],
                                         t[1008+63*j:1008+63*(j+1)], z[480+30*j:480+30*(j+1)], k[160 + 8 * j: 160 + 8 * (j + 1)], 1, round, ancilla[j], resource_check)
