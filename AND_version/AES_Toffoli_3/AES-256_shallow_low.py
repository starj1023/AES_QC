...
def AES(eng, resource_check):
    ...
    for i in range(14):
        ...
        print('Round', i)
        if (i == 1):
            Clean_ancilla(eng, t0, t1, t2, t3, k, y, t, z, w, l, 1, k_i, ancilla, resource_check)

        if (i != 0 and i != 1 and i != 13 and i % 2 == 0):
            Clean_ancilla(eng, t0, t1, t2, t3, k[0:32], y_two, t_two, z_two, w_two, l_two, 0, k_i, ancilla, resource_check)

        if (i != 0 and i != 1 and i != 13 and i % 2 == 1):
            Clean_ancilla(eng, t0, t1, t2, t3, k[128:160], y, t, z, w, l, 0, k_i, ancilla, resource_check)

        if (i != 0):
            if (i % 2 == 0):
                Keyshedule(eng, k, i, y, t, z, w, l, i, k_i, ancilla, resource_check)  # i=2, k_i=1, _one
                CNOT32(eng, k[96:128], k[64:96])
                CNOT32(eng, k[64:96], k[32:64])
                CNOT32(eng, k[32:64], k[0:32])
            else:
                Keyshedule(eng, k, i, y_two, t_two, z_two, w_two, l_two, i, k_i, ancilla, resource_check)  # i=1, k_i=0, _two, i=3, k_i=2, _two
...
def Clean_ancilla(eng, x0, x1, x2, x3, temp_k, y, t, z, w, l, flag, k_i, ancilla, resource_check):
