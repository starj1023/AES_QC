...
def Sbox(eng):
    ...
    t1 = eng.allocate_qureg(63)  # 63 * 20
    z1 = eng.allocate_qureg(30)  # 30 * 20


    if(resource_check != 1):
        Round_constant_XOR(eng, a, 0xff, n)
        Round_constant_XOR(eng, a1, 0xff, n)

    round = 1

    s = Sbox_T4(eng, a, y, t, z, s)
    #s1 = AND_Sbox_T4(eng, a1, y1, t1, z1, s1, round)
...
def AND_Sbox_T4(eng, u_in, t, m, l, s, round):
    u = []
    for i in range(8):
        u.append(u_in[7 - i])
